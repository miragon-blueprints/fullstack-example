...
CIB seven Spring-Boot-4 line, React 19, TypeScript 6, Vite 8, Tailwind 4. That is a real choice with a
real cost: newest majors have smaller ecosystems, more breaking-change churn, and occasionally force an
integration workaround (e.g. excluding the non-SB4 CIB seven webclient so the app starts on Spring 7). A
...
architecture tests, [ADR-0006](0006-mutation-testing-as-a-blocking-pr-gate.md) mutation gate, typed
...
version is exact-pinned, and each bump runs the full gate set (build, mutation ≥ 80, contract drift, FSD),
so updates can be taken continuously instead of in a scary big-bang. Bleeding-edge workarounds are
