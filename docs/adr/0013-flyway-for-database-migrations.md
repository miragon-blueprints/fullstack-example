...
[ADR-0012](0012-actuator-probes-and-prometheus-metrics.md)), and a fork can't go to production that
...
- **Tests keep Flyway off.** The `test` profile runs on in-memory H2 with Hibernate building the
  ...
  real-Postgres paths — local `bootRun` and the Bruno/Playwright CI jobs.
